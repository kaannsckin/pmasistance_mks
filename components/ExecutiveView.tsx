...
const KpiCard: React.FC<{ icon: string; label: string; value: string; sub?: string; tone?: 'default' | 'red' | 'green' }> = ({ icon, label, value, sub, tone = 'default' }) => (
  <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm px-4 py-3.5 flex items-center space-x-3 min-w-[150px]">
    ...
    <div className="leading-none min-w-0">
      ...
      <p className="text-lg font-black text-gray-800 dark:text-white leading-none truncate">{value}</p>
      ...
      <svg width={groupW * 12 + 20} height={H + 28} className="select-none">
        ...
        {capacity > 0 && (
          <>
            ...
            <text x={groupW * 12 - 2} y={y(capacity) - 4} textAnchor="end" className="fill-red-400 text-[9px] font-bold">Kapasite {fmt(capacity)}</text>
          </>
        )}
        {MONTHS_TR.map((m, i) => {
          const gx = i * groupW + 8;
          return (
            <g key={m}>
              ...
              <text x={gx + barW + 1.5} y={H + 14} textAnchor="middle" className="fill-gray-400 text-[9px] font-bold">{m}</text>
            </g>
          );
        })}
      </svg>
      <div className="flex items-center space-x-4 mt-1">
        <span className="flex items-center text-[9px] font-black text-gray-400 uppercase tracking-widest">
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-800 dark:text-white tracking-tight">Yönetim Ekranı</h2>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm p-5">
            ...
            {report.departmentPlanRows.map(d => (
              <div key={d.key}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-black text-gray-600 dark:text-gray-300">{d.label}</span>
                  <span className="text-[10px] font-black" style={{ color: 'var(--app-primary)' }}>{fmt(d.total)} AA</span>
                </div>
          ...
          {report.overAllocations.length > 0 && (
            <div className="mt-5 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl px-3 py-2.5">
              <p className="text-[9px] font-black text-red-600 dark:text-red-300 uppercase tracking-widest mb-1">Aşırı Tahsisler</p>
              <ul className="space-y-0.5">
                {report.overAllocations.slice(0, 5).map((o, i) => (
                  <li key={i} className="text-[10px] text-red-700 dark:text-red-300">
                    {o.personName} — {MONTHS_TR[o.month - 1]}: {fmt(o.total)} / {fmt(o.capacity)} AA
                  </li>
                ))}
                {report.overAllocations.length > 5 && <li className="text-[10px] font-bold text-red-500">+{report.overAllocations.length - 5} diğer</li>}
              </ul>
            </div>
          )}
        </div>
              ...
              {report.projects.map(p => (
                <tr key={p.projectId} className="hover:bg-gray-50/60 dark:hover:bg-gray-800/40 cursor-pointer" onClick={() => onOpenProject(p.projectId)} title="Projeyi aç">
                  <td className="px-4 py-2.5">
                    <span className="text-[11px] font-black text-gray-700 dark:text-gray-200">{p.name}</span>
                    {p.code && <span className="ml-2 text-[8px] font-black text-gray-400 uppercase">{p.code}</span>}
                  </td>
                  <td className="px-4 py-2.5 text-[10px] font-bold text-gray-500">{STATUS_TR[p.status]}</td>
                  <td className="px-4 py-2.5">
                    {p.rag ? (
                      <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg text-white" style={{ backgroundColor: RAG_COLORS[p.rag] }}>{RAG_TR[p.rag]}</span>
                    ) : <span className="text-[10px] text-gray-300">—</span>}
                  </td>
                  <td className="px-4 py-2.5 text-[10px] text-gray-500 dark:text-gray-400 max-w-[220px] truncate" title={p.ragNote}>{p.ragNote || '—'}</td>
                  <td className="px-4 py-2.5 min-w-[110px]">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[9px] font-black text-gray-500">%{p.progressPct}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-[10px] font-black" style={{ color: 'var(--app-primary)' }}>{fmt(p.planAA)}</td>
                  <td className="px-4 py-2.5 text-[10px] font-black text-emerald-600 dark:text-emerald-400">{fmt(p.actualAA)}</td>
                  <td className={`px-4 py-2.5 text-[10px] font-black ${p.varianceAA > 0 ? 'text-red-500' : p.varianceAA < 0 ? 'text-amber-500' : 'text-gray-400'}`}>
