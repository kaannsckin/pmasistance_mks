...
import { Project, ProjectStatus, RagStatus, TaskStatus } from '../types';
...
}> = ({ project, isActive, onOpen, onDelete, onRename, onSetRag, onSetStatus }) => {
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-2xl border shadow-sm hover:shadow-xl transition-all p-5 flex flex-col ${isActive ? 'border-primary ring-2' : 'border-gray-100 dark:border-gray-700'}`}
          ...
          <div className="flex items-center space-x-1">
            {(Object.keys(RAG_COLORS) as RagStatus[]).map(rag => (
              <button
                ...
                className={`w-4 h-4 rounded-full border-2 transition-all ${project.rag === rag ? 'scale-110 border-gray-400 dark:border-white' : 'border-transparent opacity-40 hover:opacity-100'}`}
      ...
      <input
        ...
        placeholder="Haftalık durum notu (yönetim ekranında görünür)..."
        className="w-full text-[11px] bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 rounded-xl px-3 py-2 text-gray-600 dark:text-gray-300 placeholder-gray-300 dark:placeholder-gray-600 focus:outline-none focus:border-primary mb-4"
      />
...
const PortfolioView: React.FC<PortfolioViewProps> = ({ projects, activeProjectId, onOpenProject, onCreateProject, onDeleteProject, onRenameProject, onSetRag, onSetStatus }) => {
